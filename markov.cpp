...
void findPrintAllWordProbs(GenericTrie::PrefixTree<char,double> &tree, std::string query, unsigned int n_most_probable_words){
    auto query_vec = vector<char>(query.begin(),query.end());
    ...
    std::vector<std::pair<double,unsigned int>> hr_probs;
    ...
    std::vector<std::pair<double,unsigned int>> top_ranked_probs;//truncate to most likely items if necessary
    if(hr_probs.size()>n_most_probable_words) 
        top_ranked_probs = std::vector<std::pair<double,unsigned int>>(hr_probs.end()-n_most_probable_words,hr_probs.end());
...
int main(){
    ...
    findPrintAllWordProbs(words,query,5);
